function demo_unconstrained_logistic()
...
% Modified by H.Kasai on Nov. 03, 2016
    ...
    %problem = logistic_unconstrained(data.x_train, data.y_train, data.x_test, data.y_test); 
    ...
    %[w_nt, info_nt] = newton(problem, options);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    options.subsamp_hess_size = 6553;
    options.sub_mode = 'Uniform';
    ...
    display_graph('iter','optimality_gap', {'Newton', 'Uniform', 'Leverage scroe'},...
              {w_nt, w_uni, w_ls}, {info_nt, info_uni, info_ls});          
